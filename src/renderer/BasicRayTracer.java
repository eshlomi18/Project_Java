...
public class BasicRayTracer extends RayTracerBase {
    ...
    public Color traceRay(Ray ray) {
        ...
        return closestPoint == null ? scene.background : calcColor(closestPoint, ray, MAX_CALC_COLOR_LEVEL, INITIAL_K + 5,true);
    }
     ...
     * @return
     */
    private Color calcColor(GeoPoint intersection, Ray ray, int level, double k,boolean flag) {
    ...
    private Color calcGlobalEffects(GeoPoint gp, Vector v, int level, double k) {
        ...
        if (kkr > MIN_CALC_COLOR_K)
            color = calcGlobalEffect((Ray) constructReflectedRay(gp.point, v, n), level, material.kR, kkr);

        double kkt = k * material.kT;
        if (kkt > MIN_CALC_COLOR_K)
            color = color.add(calcGlobalEffect((Ray) constructRefractedRay(gp.point, v, n), level, material.kT, kkt));
        return color;
    }
    ...
    private List<Ray> constructRefractedRay(Point3D point, Vector v, Vector n) {
        return (List<Ray>) new Ray(point, v, n);
    }
    ...
    private List<Ray> constructReflectedRay(Point3D point, Vector v, Vector n) {
        ...
        return (List<Ray>) new Ray(point, r, n);
    }

    private Color calcGlobalEffect(Ray ray, int level, double kx, double kkx) {
        GeoPoint gp = findClosestIntersection(ray);
        return (gp == null ? scene.background : calcColor(gp, ray, level - 1, kkx,true)).scale(kx);
    }
